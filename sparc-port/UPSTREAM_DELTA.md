...
| `src/add-ons/kernel/generic/ata_adapter/ata_adapter.cpp` | *pending* | Two changes. **PIO transfers carry a byte stream, not numbers:** with the accessors above converting, the data path has to convert back, or every pair of bytes on the disk is swapped. **DMA is disabled on sparc:** PCI masters address host memory through the sabre IOMMU and nothing programs it yet, so PRD entries held truncated physical addresses and the transfer landed elsewhere in memory — it corrupted the kernel. | The endianness half, **yes**. The DMA guard is a SPARC `#ifdef` and should become an IOMMU implementation rather than an upstream patch. |
