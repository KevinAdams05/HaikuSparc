...
| `.../openfirmware/devices.cpp` | *uncommitted* | `of_open(sBootPath)` opened the boot path *including its file component*, yielding an instance of a file inside a filesystem, which the partition scan then issued block reads against. Now strips the `,file` argument while keeping `:partition` — looking for the comma **after the last colon**, since OF device paths contain commas of their own (`pci@1fe,0`). | **Yes** — affects PowerPC identically; Tabby boots `hd:,\haikuloader.elf`, the same shape of path. |
